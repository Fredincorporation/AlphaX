...
// Allowed browser origins for REST + WebSocket. Comma-separated list via

// ALLOWED_ORIGIN (e.g. the Vercel frontend and the Space's own origin).

// Empty list = allow all (local development convenience).
...
function isOriginAllowed(origin: string | undefined): boolean {
  ...
  if (!origin) return true; // non-browser clients (curl, keep-alive checks)
...
// Verify the browser origin on the HTTP upgrade request before accepting the

// WS connection (cors() above does not cover WebSocket upgrades).
...
// Gated: on the Space (NODE_ENV=production) this endpoint only works when an

// ADMIN_TOKEN secret is configured and echoed in the x-admin-token header.

// Preferred setup is to set GROQ_API_KEY / GEMINI_API_KEY as Space Secrets so

// keys never pass through the browser at all.
...
app.post('/api/config/llm-key', (req, res) => {
  ...
  if (process.env.ADMIN_TOKEN) {
    ...
    return res.status(403).json({
      ...
      error: 'API key config is disabled. Set GROQ_API_KEY / GEMINI_API_KEY as Space Secrets, or set ADMIN_TOKEN to enable this endpoint.',
...
server.listen(PORT, () => {
  console.log(`\n======================================================`);
  console.log(`âš¡ AlphaX - Universal Agent Mediation Layer Server âš¡`);
